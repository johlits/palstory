...
2. **Set up the database**
   ...
   - Update database credentials in `src/html/config.php`

3. **Configure image assets**
   - Copy `src/html/config.php.example` to `src/html/config.php`
   - Copy `src/html/story/config.js.example` to `src/html/story/config.js`
...
### Database Configuration (`src/html/config.php`)
```php
$DB_SERVER = "localhost";
...
├── helper.php              # PHP helper functions
├── story/

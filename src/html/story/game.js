...
// Visibility helpers: use CSS class toggling instead of inline styles
function showEl(sel) { $(sel).removeClass('hidden'); }
function hideEl(sel) { $(sel).addClass('hidden'); }
...
bgImage = new Image();
bgImage.src = getImageUrl("wooden_bg.png");
...
var getPortrait = function (id) {
  console.log(id);
  var url = getImageUrl("p_birdman.png");
  switch (parseInt(id)) {
    case -1: url = getImageUrl("p_female_warrior.png"); break;
    case -2: url = getImageUrl("p_female_bowman.png"); break;
    case -3: url = getImageUrl("p_male_barbarian.png"); break;
    case -4: url = getImageUrl("p_male_priest.png"); break;
    case -5: url = getImageUrl("p_female_paladin.png"); break;
    case -6: url = getImageUrl("p_male_thief.png"); break;
    case -7: url = getImageUrl("p_female_mage.png"); break;
    case -8: url = getImageUrl("p_male_monk.png"); break;
  }
  console.log(url);
  return url;
}
  ...
  gc.addEventListener(
    ...
    function (evt) {
      ...
      if (Math.abs(player_x - mX) + Math.abs(player_y - mY) === 1) {
        ...
        if (!loc) {
          $(".location_name").text('???');
          hideEl("#locationStatsPrimaryBtn");
          showEl("#locationStatsDisabledBtn");
          hideEl("#locationInfoBtn");
          showEl("#locationInfoDisabledBtn");
        }
        else {
          hideEl("#locationStatsDisabledBtn");
          showEl("#locationStatsPrimaryBtn");
          hideEl("#locationInfoDisabledBtn");
          showEl("#locationInfoBtn");
        }
        hideEl("#moveDisabledBtn");
        showEl("#moveSuccessBtn");
        ...
        hideEl("#moveSuccessBtn");
        showEl("#moveDisabledBtn");
...
var toggleItemsTable = function () {
  playSound(getImageUrl("click.mp3"));
  if (itemToggle == 0) {
    itemToggle = 1;
    hideEl("#audioBtns");
    showEl("#items_table");
    ...
    success: function (response, status, http) {
      if (response.length > 0) {
        ...
        if (died) {
          ...
          playSound(getImageUrl("win.mp3"));
          document.getElementById('win-dialog').showModal();
        }
        else {
          playSound(getImageUrl("sword.mp3"));
        }
        getPlayer(false);
        move('na');
      }
      ...
      if (response.length > 0) {
        ...
        $.each(response, function (index, item) {
          ...
          $('#in' + itemId).click(function () {
            ...
            $("#item_image").attr("src", getImageUrl(itemsDict[no].image));
  ...
  $.ajax({
    ...
    success: function (response, status, http) {
      ...
      if (response.length > 0) {
        ...
        $("#monster_image").attr("src", getImageUrl(currentMonster.image));
